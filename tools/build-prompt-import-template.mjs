...
template.getRange(`A1:D${rows.length}`).values = rows;
template.freezePanes.freezeRows(1);
template.getRange('A1:D1').format.fill.color = '#0F172A';
template.getRange('A1:D1').format.font.color = '#FFFFFF';
template.getRange('A1:D1').format.font.bold = true;
template.getRange('A1:D1').format.horizontalAlignment = 'Center';
template.getRange('A1:D1').format.verticalAlignment = 'Center';
template.getRange('A1:D1').format.rowHeightPx = 32;
template.getRange('A2:D3').format.verticalAlignment = 'Top';
template.getRange('A2:D3').format.wrapText = true;
template.getRange('A:D').format.font.name = 'Arial';
template.getRange('A:D').format.font.size = 11;
...
template.getRange('A2:D2').format.rowHeightPx = 62;
template.getRange('A3:D3').format.rowHeightPx = 132;
...
const guideRows = [
  ...
  ['表头兼容', '系统也支持英文表头：category、title、description、prompt。', 'category,title,prompt'],
];
...
guide.getRange(`A1:C${guideRows.length}`).format.wrapText = true;
guide.getRange(`A1:C${guideRows.length}`).format.verticalAlignment = 'Top';
...
guide.getRange(`A2:C${guideRows.length}`).format.rowHeightPx = 58;

const inspect = await workbook.inspect({
  kind: 'table',
  range: 'Prompt导入模板!A1:D3',
  include: 'values',
  tableMaxRows: 8,
  tableMaxCols: 8,
});
...
await workbook.render({ sheetName: 'Prompt导入模板', range: 'A1:D3', scale: 1 });
await workbook.render({ sheetName: '填写说明', range: `A1:C${guideRows.length}`, scale: 1 });

...
assert.match(source, /private let targetPasteDelay: TimeInterval = 0\.18/);
assert.match(source, /DispatchQueue\.main\.asyncAfter\(deadline: \.now\(\) \+ targetPasteDelay\)/);
...
assert.match(source, /if postKeyboardShortcut\(shortcut, to: targetApp\) \{\n\s+return\n\s+\}/);
...
assert.match(source, /private func postKeyboardShortcut\(_ shortcut: TargetShortcut, to app: NSRunningApplication\? = nil\) -> Bool/);
assert.match(source, /keyDown\.postToPid\(app\.processIdentifier\)/);
assert.match(source, /keyUp\.postToPid\(app\.processIdentifier\)/);
...
assert.match(source, /URLSession\.shared\.dataTask\(with: request\)/);
